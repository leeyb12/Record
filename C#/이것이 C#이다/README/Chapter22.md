...
- C# 프로그래머들은 메모리 문제들로부터 자유롭다. CLR이 자유 메모리 관리(Automatic Memory Management) 기능을 제공한다. 자동 메모리 관리의 중심에는 **가비지 컬렉션(Garbage Collection)**이 있다. 
- 가비지 컬렉터은 "쓰레기 수거"라는 뜻으로, 더이상 사용하지 않는 객체를 수거한다.
- 이 덕분에 컴퓨터가 무한한 메모리를 가진 것처럼 간주하고 코드를 작성할 수 있게 한다.
- 가비지 컬렉터(Garage Collector)는 객체 중에 쓰레기인 것과 쓰레기가 아닌 것을 완벽하게 분리해서 쓰레기들만 조용히 수거해 간다. 
...
- 하나의 관리되는 힙(Manage Heap)을 마련하고, 할당한다.
- 다음과 같은 코드가 있다고 하자. 
  ```cs
  if ( true ) {
    object a = new object();
...
- 객체 A는 어느 코드에도 접근할 수 없게 때문에 쓰레기가 되었다. 이를 가비지 컬렉터가 수거해간다.
...
- 따라서 가비지 컬렉션을 한 번도 겪어보지 못한 갓 생산된 객체는 0세대에 위치하고, 최소 2회에서 수차례 가비지 컬렉션을 겪고도 살아남은 객체들이 위치한다. 
- 2세대의 힙이 가득 차게 되면 CLR은 애플리케이션의 실행을 잠시 멈추고 Full GC(Full Garvage Collection)을 수행함으로써 여유 메모리를 확보하려 든다. 
- 이때 애플리케이션이 차지하고 있던 메모리가 크면 클수록 Full GC 시간이 길어지므로 문제가 생길 수 있다.
...
- CLR의 객체 할당 속도가 빠르긴 하지만, 너무 많은 수의 객체는 관리되는 힙의 각 세대에 대해 메모리 포화르 ㄹ초래하고, 빈번한 가비지 컬렉션을 부를는 결과를 낳는다.
